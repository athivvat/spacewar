...
user_score_topic = '/401Chathai_topic'
...
class Button:
    ...
    def show(self, mouse=(0, 0)):
        if self.X < mouse[0] < self.X + self.width and self.Y < mouse[1] < self.Y + self.hight:
            pygame.draw.rect(screen, self.color_hoover,
                             (self.X, self.Y, self.width, self.hight))
        else:
            pygame.draw.rect(
                screen, yellow, (self.X, self.Y, self.width, self.hight))
        message_display_center(self.text, self.font,
                               black, self.centerX, self.centerY)
...
def show_high_scores():
    ...
    while high_scores_screen:
        ...
        message_display_center('High Scores', font_large, yellow, int(
            screen_sizeX/2), int(screen_sizeY * 1/10))
        message_display_center('Press (D)elete or any other key to continue',
                               font_medium, yellow, int(screen_sizeX/2), int(screen_sizeY * 9/10))
        ...
        for entry in top_5:
            ...
            index += 1
            message_display_left(str(entry[1]), font_medium, yellow, int(
                screen_sizeX * 1/8), int(screen_sizeY * (2+index)/10))
            message_display_right(str(entry[2]), font_medium, yellow, int(
                screen_sizeX * 2/4), int(screen_sizeY * (2+index)/10))
            message_display_center(str(entry[3]), font_medium, yellow, int(
                screen_sizeX * 3/4), int(screen_sizeY * (2+index)/10))
...
def menu():
    ...
    while intro_screen:
        ...
        message_display_center('SPACE WARS', font_large, yellow, int(
            screen_sizeX/2), int(screen_sizeY/3))
        message_display_center('New Game (Y/N)', font_medium,
                               yellow, int(screen_sizeX/2), int(screen_sizeY * 3/5))
        ...
        yes_button_Y = 450
        yes_button = Button(yes_button_X, yes_button_Y,
                            button_width, button_hight, 'Yes')
        ...
        no_button = Button(no_button_X,  no_button_Y,
                           button_width, button_hight, 'No')
        ...
        hs_button = Button(hs_button_X,  hs_button_Y,
                           button_width, button_hight, 'High Scores')
...
def is_collision(object1, object2):
    ...
    distance = math.sqrt(math.pow(obj1_midX-obj2_midX, 2) +
                         math.pow(obj1_midY-obj2_midY, 2))
    collision_limit = (object1.sizeX + object1.sizeY +
                       object2.sizeX + object2.sizeY) / 5
...
def show_online_score(font_size=16, x=100, y=10):
    ...
    for user_score in sorted_user_scores:
        user_exists = next(
            filter(lambda x: x['user'] == user_score['user'], top_rank), None)
        if not user_exists:
            top_rank.append(user_score)

    y_pos = y
    for user_score in top_rank:
        name_text = score_font.render(
            f"{user_score['user']}: {user_score['score']}", True, (255, 255, 0))
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type):
    ...
    message_display_center('GAME OVER', font_large, yellow, int(
        screen_sizeX/2), int(screen_sizeY * 3/20))
    message_display_center('You\'re a', font_medium, yellow, int(
...
enemy_image = [pygame.image.load(os.path.join(images_path, 'ufo_01.png')),
               ...
               pygame.image.load(os.path.join(
                   images_path, 'spaceship_03_usd.png')),
               pygame.image.load(os.path.join(
                   images_path, 'spaceship_01_usd.png')),
               pygame.image.load(os.path.join(
                   images_path, 'death_star_02.png')),
...
def save_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
    global A0, A1
    global publish_time_data, microgear, game_avg
    sending_interval = 0.5
    now = datetime.now()
    if (now - publish_time_data).seconds >= sending_interval:
        a0 = statistics.mean(A0) if len(A0) else 0
        a1 = statistics.mean(A1) if len(A1) else 0
        a2 = coin_count
        a3 = destroyed_enemy_count
        ...
        if sum([a0, a1, a2, a3, a4, a5]) != 0:
            ...
            data = dict(A0=a0, A1=a1, A2=a2, A3=a3, A4=a4, A5=a5,
...
while not quit_game:
    ...
    player = SpaceShip(
        player_image, explosion_image[0], screen_sizeX/2-32, screen_sizeY-100)
    bullet = Bullet(bullet_image, explosion_image[0], speedY=-bullet_speed,
                    sound=bullet_sound, state='hide', sizeX=32, sizeY=32)

    # initialize enemies
    enemy = []
    enemy_image_index = 0
    for i in range(num_of_enemies):
        enemy.append(SpaceEnemy(
            enemy_image[enemy_image_index], explosion_image[1], speedY=level, hit_points=level))
    ...
    for i in range(num_of_coins):
        coins.append(
            SpaceCoin(coin_image, explosion_image[0], speedY=level, hit_points=1))
